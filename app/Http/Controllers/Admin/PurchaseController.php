...
class PurchaseController extends Controller
{
    private function respondWithError($code, $message, $data)
		...
		public function editorder($p_PurchaseOrderID)
		{
			try
			{
			  $editorder=DB::select('call USP_PurchaseOrders_Get(?)',[$p_PurchaseOrderID]);
              $customer_data=DB::select('call USP_Customers_All');
			  $Suppliers_data=DB::select('call USP_Suppliers_All');
			  $Terms_data=DB::select('call USP_Terms_All');
			  $Shipping_data=DB::select('call USP_ShippingMethods_All');
			  $Shippers_data=DB::select('call USP_Shippers_All');
			  $Products_data=DB::select('call USP_Products_All');
			  $data=DB::select('SELECT COUNT(PurchaseOrderID) as abc FROM  purchaseorderdetails  where 	PurchaseOrderID='.$p_PurchaseOrderID);
			$countdata=$data[0]->abc;
			  $purchase_data = DB::select('select * from purchaseorderdetails where PurchaseOrderID='.$editorder[0]->PurchaseOrderID);

			  return view('admin/editorder',["customer_data"=> $customer_data,"Suppliers_data"=>$Suppliers_data,"Terms_data"=>$Terms_data,"Shipping_data"=>$Shipping_data,"Shippers_data"=>$Shippers_data,"Products_data"=>$Products_data,"editorder"=>$editorder,"purchase_data"=>$purchase_data,"countdata"=>$countdata]);
			}
			catch (\Exception $e)
		...
		{
			try{
                $p_PurchaseOrd=$request->PurchaseOrderID;

                $p_PurchasedetailID=$request->PurchasedetailID;
                $p_PurchasemultipleID=$request->PurchasemultipleID;
			    $data=DB::select('SELECT COUNT(	PurchaseOrderID) as abc FROM  
             purchaseorderdetails  where 	PurchaseOrderID='.$p_PurchaseOrd);
			    $ids=DB::select('select PurchaseOrderID as ids from purchaseorderdetails where PurchaseOrderID='.$p_PurchaseOrd);
			    $countdata=$data[0]->abc;
				$p_PurchaseOrderNumber=$request->purchaseordernumber;
				$p_CustomerID=$request->customer;
				$p_PurchaseOrderDescription='';
	            ...
	            $p_LoadAt=$request->loadat;
			    $p_ShipAddress2=$request->shiptocountry;
			    $p_PurchaseOrderID=$request->productcode;
			    $p_ProductID=$request->productname;
				$p_UnitPrice=$p_UnitPrice=$request->unitprice;
				$p_Quantity=$request->quantity;
				$p_Discount=$request->brand;
                $p_PurchaseOrderDetailID=''; 
				$p_PalletType=$request->pallettype;

			   DB::update('call USP_PurchaseOrders_Update(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[$p_PurchaseOrd,$p_PurchaseOrderNumber,$p_CustomerID,$p_PurchaseOrderDescription,$p_OrderDate,$p_DateRequired,$p_DatePromised,$p_ShipDate,$p_ShippingMethodID,$p_ShipToID,$p_ShipAddress,$p_ShipCity,$p_ShipRegion,$p_ShipPostalCode,$p_ShipCountry,$p_ShipPhone,$p_FreightCharge,$p_PurchaseOrderStatusID,$p_TermID,$p_TransportationBrokerID,$p_TransportationConfirmNumber,$p_Address,$p_City,$p_Region,$p_City,$p_Region,$p_Zip,$p_Phone,$p_Drop,$p_CustomerRate,$p_FreightDeductions,$p_JacketNotes,$p_ShipToPONumber,$p_SupplierID,$p_SupplierID,$p_LoadNumber,$p_LoadAt,$p_ShipAddress2]);
			    for($i=0;$i<count($p_Quantity);$i++)
		        {
			        	...
			        	DB::update('call USP_PurchaseOrderDetails_Update(?,?,?,?,?,?,?)',array($p_PurchasedetailID[$i],$p_PurchasemultipleID[$i],$p_ProductID[$i],$p_UnitPrice[$i],$p_Quantity[$i],$p_Discount[$i],$p_PalletType[$i]));
			           }
		        else
		        {
		        	DB::insert('call USP_PurchaseOrderDetails_Insert(?,?,?,?,?,?,?)',array($p_PurchaseOrd,$p_ProductID[$i],$p_UnitPrice[$i],$p_Quantity[$i],$p_Discount[$i],$p_PalletType[$i],$p_PurchaseOrderDetailID));
		        }
