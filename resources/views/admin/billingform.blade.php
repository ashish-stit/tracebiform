...
<form accept-charset="UTF-8" action="{{ url('insertbillslading') }}" method="post">
{{  csrf_field()  }}
<!-- CSRF Token -->
...
<label>Order:</label>
<select class="form-control" id="order_name">
<option>select</option>

@foreach($order_data as $data)
@foreach($orderdetail_data as $detail_data)
@foreach($customers_data as $cust_data)
@if($data->CustomerID == $cust_data->CustomerID && $detail_data->OrderID == $data->OrderID)
<option name="order" data-OrderID= "{{$data->OrderID}}" data-dilivery="{{$data->RequiredDate}}" data-diliverytime="{{$data->TimeIn}}" data-ShippedDate="{{$data->ShippedDate}}" data-LoadAt="{{$data->LoadAt}}" data-Truck="{{$data->Truck}}" data-ShipToPONumber="{{$data->ShipToPONumber}}" data-PalletsIn="{{$data->PalletsIn}}" data-PalletsOut="{{$data->PalletsOut}}" data-LoadNumber="{{$data->LoadNumber}}" data-Freight="{{$data->Freight}}" data-Instructions="{{$data->Instructions}}" data-TrailerLicense="{{$data->TrailerLicense}}" data-TrailerLicense="{{$data->TrailerLicense}}" data-OrderID
...
<br>
<textarea class="form-control" placeholder="Delivery Instructions" id="Instructions" name="DeliveryInstructions">
</textarea>
</div>
...
<div class="col-xs-12 col-sm-2 col-md-2">\
<label>Date Packed :</label> <input type="text" value=" '+value.TransactionDate+'">\<a href="#"  class="del">Remove</a>\
</div></div>');
});
