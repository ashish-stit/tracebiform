	...
	<li class="active">Add Plant Application</li>
	</ol>
	</section>
	<div class="container">
	...
	<h3>Add Plant Application</h3>
	</div>
	<div class="card-body">
	<!-- display all errors here -->
	<form accept-charset="UTF-8" action="{{ url('admin/saveapplication') }}" method="post">
	{{  csrf_field()  }}
	<!-- CSRF Token -->
	...
	<option name= "GrowerFieldID" value="{{ $application->FarmFieldPlantID }}">
	...
	<input class="form-control" placeholder="Date Time Sprayed" name="DateTimeSprayed" type="date"
	value=""/>
	</div>
	</div>
	...
	<input class="form-control" placeholder="Field Scheduled ReEnter Time" name="FieldScheduledReEnterTime" type="date"
	value=""/>
	</div>
	</div>
	...
	@foreach($pest_data as $pet_data)
	<option name= "petsname" data-id= "{{$pet_data->PestID}}" data-petsname= "{{$pet_data->PestName}}" value="{{$pet_data->PestID}}">
	{{$pet_data->PestName}}
	</option>
	@endforeach
	</select>
