...
<script>
   $(document).ready(function() {
      ...
      $(add_button).click(function(e) {
         ...
         if (x < max_fields) {
               x++;
              $(wrapper).append('<div><input type="text" name="quantity[]"/><select name="wrapper"> @foreach($supplies as $supplies_data) <option value="{{$supplies_data->SupplyID}}">{{$supplies_data->Description}}</option> @endforeach</select><a href="#"  class="delete">Remove</a></div>'); 
         } else {
            ...
            <div class="col-10 col-offset-1 mx-auto">
                <div class="card ">
                    ...
                    <div class="card-body">
                        ...
                        <form accept-charset="UTF-8" action="{{ url('product/saveitem') }}" method="post">
                            {{  csrf_field()  }}
                            <!-- CSRF Token -->
                            ...
                            <div class="container1" style="margin-top: 20px;">
							    <button class="add_form_field btn btn-primary">Add Supply </span>
							    </button>
							    
							    <div style="margin-top: 20px;">
							      <input type="text" name="quantity[]">
							      <select name="wraper">
							      	@foreach($supplies as $supplies_data)
							         <option value="{{$supplies_data->SupplyID}}">{{$supplies_data->Description}}</option>
							        @endforeach
							      </select>
							    </div>
							</div>
